...
import { PhotoAsset, RootStackParamList } from '../../App'; // Importamos PhotoAsset
...
const HomeScreen = ({ navigation }: Props) => {
  ...
  const [selectedModel, setSelectedModel] = useState<'yolov8' | 'yolov11'>(
    'yolov11',
  );
  const [threshold, setThreshold] = useState(0.5); // Threshold de confianza (50% por defecto)
  ...
  const handleOpenGallery = async () => {
    ...
    if (!hasPermission) {
      Alert.alert(
        'Permiso Requerido',
        "Habilita 'Permitir Todo' en configuración.",
        [
  ...
  const handleAnalyzeGallery = () => {
    ...
    console.log(`🟢 [HomeScreen] Threshold de confianza: ${threshold}`);
    console.log(
      '🟢 [HomeScreen] Datos de fotos:',
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
        <View style={styles.header}>
          ...
          <View style={styles.badge}>
            <Text style={styles.badgeText}>Offline v0.0.4</Text>
          </View>
        </View>
            ...
            {['yolov8', 'yolov11'].map(m => (
              ...
              >
                <Text
                  style={[
                    styles.modelTxt,
                    selectedModel === m && styles.modelTxtActive,
                  ]}
                >
                  {m.toUpperCase()}
                </Text>
        ...
        {/* Slider de Threshold de Confianza */}
        <View style={styles.section}>
          ...
          <View style={styles.thresholdInfo}>
            <Text style={styles.thresholdLabel}>📊 Bajo (más detecciones)</Text>
            <Text style={styles.thresholdLabel}>Alto (más precisas) 🎯</Text>
          </View>
          <Text style={styles.thresholdDesc}>
            Ajusta qué tan seguro debe estar el modelo para marcar una plaga
          </Text>
        </View>
        ...
        <View style={styles.section}>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate('Detection', { modelName: selectedModel })
            }
          >
            <Text style={styles.emoji}>📷</Text>
            <View>
              <Text style={styles.cardTitle}>Nueva Captura</Text>
              <Text style={styles.cardSub}>Usar cámara</Text>
            </View>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            ...
            <View>
              ...
              <Text style={styles.cardSub}>Galería (50MP)</Text>
            </View>
          </TouchableOpacity>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  slider: { width: '100%', height: 60, marginVertical: 10 },
  thresholdValue: { color: '#15803d', fontWeight: 'bold', fontSize: 16 },
  thresholdInfo: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginTop: -10,
  },
  thresholdLabel: { fontSize: 11, color: '#64748b' },
  thresholdDesc: {
    fontSize: 12,
    color: '#94a3b8',
    marginTop: 5,
